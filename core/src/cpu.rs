...
fn to_joypad_button(button: button::Button) -> joypad::Button {
    match button {
        button::Button::Joypad1A | button::Button::Joypad2A => joypad::Button::A,
        button::Button::Joypad1B | button::Button::Joypad2B => joypad::Button::B,
        button::Button::Joypad1Up | button::Button::Joypad2Up => joypad::Button::Up,
        button::Button::Joypad1Down | button::Button::Joypad2Down => joypad::Button::Down,
        button::Button::Joypad1Left | button::Button::Joypad2Left => joypad::Button::Left,
        button::Button::Joypad1Right | button::Button::Joypad2Right => joypad::Button::Right,
...
pub fn operation(opc: u8) -> Operation {
    match opc {
        0x00 => Operation {
...
impl Cpu {
    ...
    fn operate(&mut self, op: &Operation) {
        match op.instruction_type {
            ...
            InstructionTypes::ASL => {
                let result =
                    self.update_memory_with_addressing_mode(op, |src: u8| (src as u16) << 1);
                self.update_n(result);
                self.update_z(result);
                self.update_c(result);
            }
            ...
            InstructionTypes::CMP | InstructionTypes::CPX | InstructionTypes::CPY => {
                let src1 = match op.instruction_type {
                    InstructionTypes::CMP => self.a.load(),
                    InstructionTypes::CPX => self.x.load(),
                    _ => {
                        //InstructionTypes::CPY
                        self.y.load()
                    }
                };
                let src2 = self.load_with_addressing_mode(&op);
            ...
            InstructionTypes::DEC => {
                let result = self
                    .update_memory_with_addressing_mode(op, |src: u8| (src as u16).wrapping_sub(1));
                self.update_n(result);
                self.update_z(result);
            }
            ...
            InstructionTypes::INC => {
                let result = self
                    .update_memory_with_addressing_mode(op, |src: u8| (src as u16).wrapping_add(1));
                self.update_n(result);
                self.update_z(result);
            }
    ...
    fn load_2bytes_from_zeropage(&mut self, address: u16) -> u16 {
        self.ram.load((address & 0xff) as u32) as u16
            | ((self.ram.load((address.wrapping_add(1) & 0xff) as u32) as u16) << 8)
    }
    ...
    fn load_with_addressing_mode(&mut self, op: &Operation) -> u16 {
        match op.addressing_mode {
            AddressingModes::Accumulator => self.a.load() as u16,
            _ => {
                let address = self.get_address_with_addressing_mode(&op);
    ...
    fn update_memory_with_addressing_mode<F>(&mut self, op: &Operation, func: F) -> u16
    where
        F: Fn(u8) -> u16,
    {
        match op.addressing_mode {
    ...
    pub fn dump(&mut self) -> String {
        ...
        "p:".to_owned()
            ...
            + &self.y.dump()
            + &" ".to_owned()
            + instruction_name(op.instruction_type)
            + &" ".to_owned()
    ...
    fn dump_addressing_mode(&mut self, mode: AddressingModes, pc: u16) -> String {
        match mode {
            AddressingModes::Immediate => {
                "#".to_owned() + &format!("0x{:02x} ", self.load(pc)) + &"immediate".to_owned()
            }
            AddressingModes::Relative => {
                format!("0x{:02x} ", self.load(pc) as i8) + &"relative".to_owned()
            }
            AddressingModes::Absolute => {
            ...
            AddressingModes::IndexedAbsoluteX => {
                ...
                format!("0x{:04x},X ", address)
                    + &format!(
                        "(0x{:02x}) ",
                        self.load((self.x.load() as u16).wrapping_add(address)) as i8
                    )
            ...
            AddressingModes::IndexedAbsoluteY => {
                ...
                format!("0x{:04x},Y ", address)
                    + &format!(
                        "(0x{:02x}) ",
                        self.load((self.y.load() as u16).wrapping_add(address)) as i8
                    )
            ...
            AddressingModes::IndexedZeroPageX => {
                ...
                format!("0x{:02x},X ", address)
                    + &format!(
                        "(0x{:02x}) ",
                        self.load(self.x.load().wrapping_add(address) as u16) as i8
                    )
            ...
            AddressingModes::IndexedZeroPageY => {
                ...
                format!("0x{:02x},Y ", address)
                    + &format!(
                        "(0x{:02x}) ",
                        self.load(self.y.load().wrapping_add(address) as u16) as i8
                    )
            ...
            AddressingModes::IndexedIndirectY => {
                let address = self.load(pc) as u16;
                let address2 = self
                    .load_2bytes_from_zeropage(address)
                    .wrapping_add(self.x.load() as u16);
                format!("0x{:02x},Y ", address)
                    + &format!("(0x{:04x}(0x{:02x})) ", address2, self.load(address2) as i8)
                    + &"indexed_indirect_y".to_owned()
            }
            AddressingModes::Accumulator => {
                format!("A0x{:02x} ", self.a.load()) + &"accumulator".to_owned()
            }
            _ => "".to_owned(),
        }
    }
